...
const AdminDashboard = () => {
    ...
    const exportToPDF = () => {
        ...
        try {
            ...
            doc.text('MediCare Online Pharmacy & Channeling System', pageWidth / 2, currentY, { align: 'center' });
            currentY += 10;
            
            doc.setFontSize(16);
            doc.text('User Management Report', pageWidth / 2, currentY, { align: 'center' });
            currentY += 15;
            
            // Generated by info
            doc.setFontSize(10);
            doc.setFont('helvetica', 'normal');
            doc.text(`Generated By: ${user?.firstName || ''} ${user?.lastName || ''} / ${user?.email || ''}`, 14, currentY);
            currentY += 6;
            ...
            doc.text(`Generated Date & Time: ${formattedDate} ${formattedTime}`, 14, currentY);
            currentY += 15;
            ...
            doc.setFontSize(10);
            doc.setFont('helvetica', 'normal');
            doc.text(`Total Users Count: ${totalUsers}`, 14, currentY);
            currentY += 6;
            doc.text(`Active Users: ${activeUsers}`, 14, currentY);
            currentY += 6;
            doc.text(`Inactive Users: ${inactiveUsers}`, 14, currentY);
            currentY += 6;
            
            // Role distribution
            doc.text('User Role Distribution:', 14, currentY);
            currentY += 6;
            
            Object.entries(roleCounts).forEach(([role, count]) => {
                doc.text(`- ${role}: ${count}`, 24, currentY);
                currentY += 6;
            });
            ...
            doc.setFontSize(14);
            doc.setFont('helvetica', 'bold');
            doc.text('User Summary Table', 14, currentY);
            currentY += 10;
            
            // Prepare data for table
             const tableData = filteredUsers.map(user => [
                 user._id ? user._id.slice(-6) : 'N/A', // User ID (last 6 chars)
             ...
             // Add table
             autoTable(doc, {
                 head: [['User ID', 'Full Name', 'Email Address', 'Phone Number', 'Role', 'Status', 'Created Date']],
                body: tableData,
                startY: currentY,
                styles: {
                    fontSize: 8,
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                // Footer text
                doc.text('Generated automatically by MediCare system', 14, doc.internal.pageSize.height - 15);
            ...
            toast.success('User Management Report exported to PDF successfully!');
        } catch (error) {
