                                ...
                                {emailLogs.map(log => (
                                    <tr key={log._id} className="border-t">
                                        ...
                                        <td className="p-2 text-xs">
                                            <button
                                                title="Delete log"
                                                className="inline-flex items-center gap-1 rounded-md border border-red-200 bg-red-50 px-2 py-1 text-red-600 hover:bg-red-100 hover:text-red-700 active:bg-red-200 transition"
                                                onClick={async () => {
                                                    if (!window.confirm('Delete this email log?')) return;
                                                    try {
                                                        const res = await fetch(`http://localhost:5001/api/products/reorder/logs/${log._id}` , {
                                                            method: 'DELETE',
                                                            headers: { 'Authorization': `Bearer ${token}` }
                                                        });
                                            ...
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
                                                    <path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 100 2h.293l.853 9.383A2 2 0 007.138 17h5.724a2 2 0 001.992-1.617L15.707 6H16a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zm-1.95 4l.75 8.25a.75.75 0 001.493-.134L8.543 6H7.05zm5.9 0l-.75 8.25a.75.75 0 001.493.134L12.95 6h-1.493z" clipRule="evenodd" />
                                                </svg>
...
function OrdersSection() {
    ...
    // Build public URL for stored prescription files
    // Helper is defined later to handle both filename and full path variants
                                ...
                                <tr key={o._id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="p-3">
                                        ...
                                        <div className="text-xs text-gray-500 max-w-xs truncate" title={o.orderType === 'prescription' ? o.prescriptionDetails?.patientAddress : o.customer?.address}>
                                          ...
                                          {o.prescriptionFile && (
                                              <div className="mt-1 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-purple-100 text-purple-800">
            ...
            {selected && (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    ...
                    <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl shadow-lg p-6">
                        ...
                        {/* Uploaded Prescription Preview (always below product list section when available) */}
                        {selected?.prescriptionFile && (
                            <div className="mb-6">
                                <h3 className="font-semibold mb-3">Uploaded Prescription</h3>
                                <div className="bg-gray-50 rounded-lg p-4">
                                    {String(selected.prescriptionFile).toLowerCase().endsWith('.pdf') ? (
                                        <div>
                                            <a
                                                href={getPrescriptionFileUrl(selected.prescriptionFile)}
                                                target="_blank"
                                            ...
                                            <div className="mt-3">
                                                <iframe
                                                    ...
                                                    src={getPrescriptionFileUrl(selected.prescriptionFile)}
                        ...
                        {/* Prescription Description (patient-provided) */}
                        {(selected?.prescriptionDetails?.notes || selected?.customer?.notes) && (
                            <div className="mb-6">
                                <h3 className="font-semibold mb-3">Prescription Description</h3>
